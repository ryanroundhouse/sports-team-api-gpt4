...
const setupPlayerRoutes = (db: Database) => {
  ...
  router.get("/players/:id/team-memberships", authenticate, (req, res) => {
    logger.info(`GET /players/${req.params.id}/team-memberships`);
    teamMembershipController.readManyByUserId(req, res, db);
  });
