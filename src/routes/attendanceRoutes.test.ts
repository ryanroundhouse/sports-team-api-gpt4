...
describe('POST /attendance', () => {
  ...
  let nonTeamMemberPlayerId: number;
  let teamId: number;
  let gameId: number;
  ...
  it('should create attendance if the user is authenticated', async () => {
    ...
    expect(res.body.playerId).toEqual(captainId);
    expect(res.body.gameId).toEqual(gameId);
...
describe('GET /attendance', () => {
  ...
  let teamId: number;
  let gameId: number;
  let captainAttendanceId: number;
  let nonCaptainAttendanceId: number;
  ...
  beforeEach(async () => {
    ...
    captainAttendanceId = (
      (await createAttendance(db, captainAttendance)) ?? { id: -1 }
    ).id;
    ...
    nonCaptainAttendanceId = (
      (await createAttendance(db, nonCaptainAttendance)) ?? { id: -2 }
    ).id;
  });
  afterEach(async () => {
    await db.run('DELETE FROM teams');
  ...
  it('should get all attendances if the user is authenticated', async () => {
    ...
    expect(res.body[0].playerId).toEqual(captainId);
    expect(res.body[0].gameId).toEqual(gameId);
    expect(res.body[0].status).toEqual('present');
    expect(res.body[1].playerId).toEqual(nonCaptainId);
    expect(res.body[1].gameId).toEqual(gameId);
